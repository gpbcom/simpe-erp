...
class CertificationTypeService:
    ...
    Notes:
        - **This service owns the referential integrity the database does
          not.** A required qualification is a JSON array on
          ``intervention_types`` and an optional string on ``certifications``,
          and a foreign key can constrain neither the first nor, usefully, the
          second. :meth:`assert_known` is what every writer of a requirement
          calls, and :meth:`delete` is what refuses to strand one.
        - Retiring is the ordinary way to take a qualification out of use.
          Deleting is offered only while nothing refers to the entry, which in
          practice means the morning it was added by mistake.
    """

    def __init__(
        self,
        certifications: CertificationTypeRepository,
        hcas: HcaRepository,
        ...
        self.logger.debug("CertificationTypeService created.")
    ...
    async def _holders_of(self, code: str) -> List[str]:
        """Return the assistants holding a qualification.

        Args:
            code (str): The catalogue code to look for.

        Returns:
            List[str]: The full names of the assistants who hold it.
        ...
        return [
            ...
            if any(held.code == code for held in assistant.certifications)
        ]

    async def _services_requiring(self, code: str) -> List[str]:
        """Return the catalogue entries that require a qualification.
        ...
        return [
            ...
            if code in service.required_certification_codes
        ]
    ...
    async def assert_known(self, codes: Sequence[str]) -> None:
        ...
        Notes:
            - **This is the referential integrity the JSON column cannot have.**
              ...
              instead — and it produces a better message than a constraint would:
        ...
        """
        if not codes:
            self.logger.debug("No certification requirement to check.")
            return
        wanted = {code.strip().upper() for code in codes}
        known: Set[str] = await self.certifications.known_codes()
        unknown = sorted(wanted - known)
        if not unknown:
            self.logger.debug(
                "Every required certification code is in the catalogue: %s.",
                ", ".join(sorted(wanted)),
            )
            return
        self.logger.warning(
            "Refused a requirement naming %d unknown certification code(s): %s.",
            len(unknown),
            ", ".join(unknown),
        )
        raise MTCertificationTypeUnknownCode(
            f"Unknown certification code(s): {', '.join(unknown)}. "
    ...
    async def create(self, certification_type: CertificationType) -> CertificationType:  # noqa: E501
        ...
        Raises:
            MTCertificationTypeAlreadyExists: If the code is already taken.
        """
        self.logger.info(
            "Adding certification type %s to the catalogue.",
            certification_type.code,
        )
        try:
            return await self.certifications.create(certification_type)
        except IntegrityError as exc:
            self.logger.warning(
                "Refused to add %s: the code is already used.",
                certification_type.code,
            )
            raise MTCertificationTypeAlreadyExists(
                f"A certification already uses the code {certification_type.code!r}."  # noqa: E501
            ) from exc

    async def get(self, type_id: str) -> CertificationType:
        ...
        Raises:
            MTCertificationTypeNotFound: If no such entry exists.
        """
        found = await self.certifications.get(type_id)
        if found is None:
            self.logger.warning("Certification type %s does not exist.", type_id)  # noqa: E501
            raise MTCertificationTypeNotFound(
                f"No certification type {type_id!r} exists."
            )
        return found
    ...
    async def list(
        ...
        """
        self.logger.debug(
            "Listing the certification catalogue: page=%d include_inactive=%s.",
            page,
            include_inactive,
        )
    ...
    async def update(
        ...
        stored = await self.certifications.update(updated)
        if stored is None:
            self.logger.error(
                "Certification type %s vanished between the read and the write.",
                type_id,
            )
            raise MTCertificationTypeNotFound(
                f"No certification type {type_id!r} exists."
            )
        self.logger.info("Updated certification type %s (%s).", type_id, stored.code)  # noqa: E501
        return stored
    ...
    async def delete(self, type_id: str) -> None:
        ...
        Notes:
            ...
            - The refusal names *what* still refers to the entry, and says
              ...
              message somebody works around by deleting the assistant's
        ...
        if holders or services:
            self.logger.warning(
                "Refused to delete certification %s: %d holder(s) and "
                "%d service(s) still refer to it.",
                existing.code,
                len(holders),
                len(services),
            )
            raise MTCertificationTypeInUse(
                f"The certification {existing.code!r} is still in use by "
                f"{len(holders)} assistant(s) and {len(services)} service(s). "
                f"Retire it instead, which stops it being required again "
                f"without stranding the qualifications that name it."
            )
        removed = await self.certifications.delete(type_id)
        if not removed:
            self.logger.error(
                "Certification type %s vanished between the read and the delete.",
                type_id,
            )
            raise MTCertificationTypeNotFound(
                f"No certification type {type_id!r} exists."
            )
        self.logger.info("Deleted certification type %s (%s).", type_id, existing.code)

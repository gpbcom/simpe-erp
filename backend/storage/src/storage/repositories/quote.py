...
from storage.mappers.quote_mapper import QuoteMapper
...
class QuoteRepository(BaseRepository[QuoteRow]):
    ...
    def __init__(self, session: AsyncSession, logger: Optional[Logger] = None) -> None:
        ...
        super().__init__(session=session, row_class=QuoteRow, logger=resolved_logger)  # noqa: E501
    ...
    async def list_schedulable(
        ...
        """Return the accepted quotes with work inside a period.
        ...
        Returns:
            List[Quote]: Every accepted quote with at least one line in range.
        ...
        """
        self.logger.info(
            "Loading schedulable quotes between %s and %s.",
            period_start,
            period_end,  # noqa: E501
        )
        statement = (
            select(QuoteRow)
            .join(QuoteLineRow, QuoteLineRow.quote_id == QuoteRow.id)
        ...
        if not rows:
            self.logger.warning(
                "No accepted quote covers %s to %s; a planning run would have "
                "nothing to schedule.",
                period_start,
                period_end,
            )

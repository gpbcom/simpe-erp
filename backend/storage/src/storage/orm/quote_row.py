...
if TYPE_CHECKING:
    # First-party imports
    from storage.orm.quote_aggregate_row import QuoteAggregateRow
    from storage.orm.quote_line_row import QuoteLineRow
...
class QuoteRow(Base):
    ...
    # Wide enough for 'pending-validation', which is 18 characters. The column
    # was String(16) when the longest status was 'accepted'; a status that no
    # longer fits is a truncation on SQLite and an error on PostgreSQL, and the
    # difference between the two is a bug that only appears in production.
    status: Mapped[str] = mapped_column(String(32), nullable=False)
    issued_on: Mapped[Optional[date]] = mapped_column(Date, nullable=True)
    valid_until: Mapped[Optional[date]] = mapped_column(Date, nullable=True)
    # Both carry an account identifier without a foreign key, for the same
    # reason ``planning_runs.requested_by`` does: this is an audit trail, and
    # somebody leaving the agency must not take the record of what they wrote
    # and approved with them, nor block their account being deleted.

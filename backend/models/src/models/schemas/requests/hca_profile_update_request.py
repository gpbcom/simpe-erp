...
class HcaProfileUpdateRequest(BaseModel):
    ...
    Notes:
        - **What is absent is the point.** There is no ``contract_type``, no
          ``certifications`` and no role. An assistant does not decide what they
          are qualified to do or what they are employed as — a manager does,
          through ``PATCH /api/v1/hcas/{id}/employment``. Leaving those fields
          out of the payload means they cannot be smuggled in, rather than
          relying on an endpoint remembering to ignore them.

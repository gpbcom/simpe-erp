...
class CertificationTypeUpdateRequest(BaseModel):
    """The payload changing what a certification-catalogue entry says.

    Attributes:
        label (Optional[str]): The display name.
        description (Optional[str]): What the qualification is.
        is_active (Optional[bool]): Whether it may still be required or
            recorded.

    Notes:
        **``code`` is absent, so it cannot be changed at all.** It is what
        every assistant's stored qualification and every intervention type's
        requirement is matched on; renaming it would leave a workforce holding
        certifications for a code that no longer exists and quietly disqualify
        all of them on the next planning run. The screen shows the field
        locked, but the rule lives here — a locked input is a courtesy, not a
    ...
    description: Optional[str] = Field(
        default=None, description="What the qualification is."
    )
    is_active: Optional[bool] = Field(
        default=None, description="Whether it may still be required or held."
    )
    ...
    def validate_label(cls, value: Optional[str]) -> Optional[str]:
        ...
        Raises:
            MTCertificationTypeUpdateRequestInvalidLabel: If ``value`` is
                present but blank once stripped, or is not a string.
        ...
        Notes:
            ...
            one without it shows on the workforce screen as an empty chip
            nobody can identify.
        """
        if value is None:
            return None
        if not isinstance(value, str) or not value.strip():
            raise MTCertificationTypeUpdateRequestInvalidLabel(
    ...
    def validate_description(cls, value: Optional[str]) -> Optional[str]:
        ...
        Raises:
            MTCertificationTypeUpdateRequestInvalidDescription: If ``value`` is
                neither ``None`` nor a string.
        """
        if value is None:
            return None
        if not isinstance(value, str):
            raise MTCertificationTypeUpdateRequestInvalidDescription(
    ...
    def validate_is_active(cls, value: Union[bool, str, int, None]) -> Optional[bool]:
        ...
        Raises:
            MTCertificationTypeUpdateRequestInvalidIsActive: If ``value`` is
                neither ``None`` nor a boolean.

        Notes:
            Strings are refused rather than coerced. ``"false"`` is truthy, and
            a retirement read as "still in use" would leave an obsolete
            qualification on offer with nothing on screen to say the request
            had not taken.
        """
        if value is None:
            return None
        if not isinstance(value, bool):
            raise MTCertificationTypeUpdateRequestInvalidIsActive(

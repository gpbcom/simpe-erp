...
class CertificationTypeFilter(EntityFilter):
    """What narrows the certification catalogue on the way out of the API.
    ...
    def validate_text(cls, value: Optional[str]) -> Optional[str]:
        ...
        Raises:
            MTCertificationTypeFilterInvalidFragment: If ``value`` is neither ``None``
                nor a string.
        """
    ...
    def validate_flags(cls, value: Union[bool, str, int, None]) -> Optional[bool]:
        ...
        Raises:
            MTCertificationTypeFilterInvalidFlag: If ``value`` is neither ``None`` nor a
                boolean.
        """

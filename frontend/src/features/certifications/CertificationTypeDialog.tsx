...
interface CertificationTypeDialogProps {
  /** The entry to edit, or `null` when not editing one. */
  entry: CertificationType | null;
...
/**
 * Add a qualification to the catalogue, or change what an existing one says.
 *
 ...
 *
 * **`code` is fixed once an entry exists**, and this is the field that matters
 * most on the screen. It is what every assistant's stored qualification and
 * every service's requirement is matched on, so renaming it would disqualify
 * every holder on the next planning run. The server refuses it outright — the
 * payload carries no `code` at all — and the input is locked with the reason
 * beneath it. A locked field that does not explain itself reads as a bug.
 *
 * **Deleting is offered only from here, and usually refused.** The server
 * counts the assistants holding the code and the services requiring it and
...
export function CertificationTypeDialog({
  ...
  const update = useUpdateCertificationType();
  const create = useCreateCertificationType();
  const remove = useDeleteCertificationType();
  const [form, setForm] = useState<CertificationForm>(EMPTY);
  ...
  return (
    ...
    >
      <DialogTitle>
        {entry ? t('certifications.edit') : t('certifications.add')}
      </DialogTitle>

      <DialogContent dividers>
        <Stack spacing={2} sx={{ mt: 1 }}>
          {error ? (
            <Alert severity="error" data-testid="certification-dialog-error">
              {error}
            </Alert>
          ) : null}
          ...
          <Grid container spacing={2}>
            <Grid size={{ xs: 12, sm: 4 }}>
              <TextField
                label={t('certifications.code')}
                value={form.code}
                onChange={(event) => setForm({ ...form, code: event.target.value })}
                disabled={Boolean(entry)}
                error={form.code.trim() !== '' && !codeIsWellFormed}
                helperText={
                  entry ? t('certifications.codeIsFixed') : t('certifications.codeHint')
                }
                slotProps={{ htmlInput: { 'data-testid': 'certification-code' } }}
              />
            </Grid>
            <Grid size={{ xs: 12, sm: 8 }}>
              <TextField
                ...
                slotProps={{ htmlInput: { 'data-testid': 'certification-label' } }}
              />
            </Grid>
            <Grid size={12}>
              <TextField
                ...
                rows={2}
                slotProps={{
                  htmlInput: { 'data-testid': 'certification-description' },
                }}
              />
            </Grid>
          </Grid>
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                data-testid="certification-active"
              />
            }
            label={t('certifications.active')}
          />
          <Typography variant="caption" color="text.secondary">
            {t('certifications.retiredExplained')}
          </Typography>
        </Stack>
      </DialogContent>
      ...
      <DialogActions sx={{ px: 3, py: 2 }}>
        {entry ? (
          <Button
            ...
            data-testid="delete-certification"
          >
            {t('common.delete')}
          </Button>
        ) : null}
        <Box sx={{ flexGrow: 1 }} />
        <Button onClick={onClose} data-testid="cancel-certification">
          {t('common.cancel')}
        </Button>
        <Button
          variant="contained"
          ...
          data-testid="save-certification"
        >
          {t('common.save')}
        </Button>
      </DialogActions>
